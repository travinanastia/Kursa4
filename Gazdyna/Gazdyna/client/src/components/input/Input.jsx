...
const Input = ({
  ...
  return (
    <div className={`flex flex-col relative ${fullWidth ? 'w-full' : null}`}>
      <label htmlFor={id} className='text-sm font-semibold mb-3'>
        {label}
      </label>
      <span className='absolute top-[39px] left-3 bg-light p-1 rounded text-primary'>{icon}</span>
      <input
      ...
      <span id={`${id}-error`} className='hidden text-red-500 pl-2 text-sm mt-1'>
        {errorMessage}
      </span>
    </div>
  );
